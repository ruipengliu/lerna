...
stage websocket go test -mod=readonly -p 1 -race -count=1 -timeout=3m ./adapters/transport/ws ./profiles/asynccheck -run 'Test(WS|FiniteConfiguration)' -json > build/ws/websocket.jsonl 2> build/ws/websocket.stderr
stage regression go test -mod=readonly -p 1 -race -count=1 -timeout=10m ./authorization ./execution ./catalog ./schema ./protocol ./sdk ./adapters/transport/nodetls ./adapters/catalog/auth ./adapters/catalog/local ./adapters/catalog/sqlite ./adapters/transport/grpc ./profiles/sdkcontract ./profiles/catalogcheck ./profiles/asynccheck -json > build/ws/regression.jsonl 2> build/ws/regression.stderr
