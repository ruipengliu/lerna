// Package fetchexecution connects an admitted Execution call to acquisition.
// Only Execution may invoke this trusted seam after Core qualification checks.
package fetchexecution
...
func (d *Driver) input(raw []byte) (fetch.Request, error) {
...
func (d *Driver) WithObservations(scope ObservationScope) (*Driver, error) {
	if scope == nil {
		return nil, fetch.Invalid
	}
	copy := *d
...
func (d *Driver) WithRecovery(scope ObservationScope, guard TaskGuard) (*Driver, error) {
	if guard == nil {
		return nil, fetch.Invalid
	}
