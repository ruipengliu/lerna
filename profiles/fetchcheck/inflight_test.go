...
func TestTaskControlInterruptsInFlightAcquisition(t *testing.T) {
	for _, intent := range []string{"PAUSE", "CANCEL"} {
		t.Run(intent, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.URL.Path == "/start" {
					...
					case <-time.After(3 * time.Second):
						t.Error("HTTP acquisition did not stop")
					}
					return
				}
			...
			if err != nil || !known || outcome.Status != "denied" || outcome.Requests != 1 || outcome.Reference != "" || hits.Load() != 1 || result.Result == "SUCCESS" {
				t.Fatalf("task control did not interrupt acquisition: %+v known=%v hits=%d result=%s err=%v", outcome, known, hits.Load(), result.Result, err)
			}
		})
	}
}
