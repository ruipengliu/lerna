...
import (
	...
	"lerna/adapters/execution/simapi"
	"lerna/answers"
	"lerna/artifacts"
	"lerna/authorization"
	"lerna/execution"
...
func capability() execution.Capability {
	return execution.Capability{Name: "counter.add", Version: "1", Implementation: "sqlite-counter", ImplementationVersion: "1", Resource: "root", Purpose: "task", Location: "local", Exclusive: true, Synchronous: true,
...
func open(ctx context.Context, root, token string) (h *harness, err error) {
	...
	h.target, err = simapi.Open(filepath.Join(root, "target.db"))
	if err != nil {
		return nil, err
	}
