// Package control verifies durable control and simulated in-flight disposition.
package control
...
func scope(now time.Time, execute bool) *wire.AuthorizationScope {
	actions := []string{"task.submit", "task.read", "task.pause", "task.cancel", "task.resume", "task.reconcile"}
	if execute {
...
func controlLimits() tasks.ControlLimits {
	return tasks.ControlLimits{MaxOperations: 32, MaxObservations: 32, MaxChecks: 4, PollInterval: 10 * time.Millisecond, StopTimeout: 30 * time.Millisecond, IOTimeout: time.Second}
}
func (h *harness) controls() (*tasks.ControlService, error) {
	return h.service.Controls(controlLimits())
}
func (h *harness) request(ctx context.Context, t tasks.Task, intent string) (tasks.ControlRequest, error) {
	id, err := h.auth.NewOperation(ctx, h.token)
	return tasks.ControlRequest{Ref: t.Ref, OperationID: id, Intent: intent, ExpectedVersion: t.Version}, err
}
func (h *harness) control(ctx context.Context, t tasks.Task, intent string) (tasks.ControlReceipt, error) {
	c, err := h.controls()
	if err != nil {
		return tasks.ControlReceipt{}, err
	}
	in, err := h.request(ctx, t, intent)
	if err != nil {
		return tasks.ControlReceipt{}, err
	}
	return c.Request(ctx, h.token, in)
}
func (h *harness) start(ctx context.Context, s tasks.RunSnapshot, effectAware bool) (*tasks.WorkPort, tasks.RunSnapshot, error) {
	p, err := h.service.BindWorker(tasks.WorkerBinding{Token: h.token, Subject: "operator", WorkerID: "worker-a", AllowEffectEvidence: effectAware}, limits())
	if err != nil {
		return nil, tasks.RunSnapshot{}, err
	}
	claim, err := p.Commit(ctx, mutation("claim", "claim", s))
	if err != nil {
		return nil, tasks.RunSnapshot{}, err
	}
	started, err := p.Commit(ctx, mutation("start", "start", claim))
	return p, started, err
}
