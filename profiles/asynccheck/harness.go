...
import (
	...
	"lerna/adapters/execution/simjob"
	"lerna/answers"
	"lerna/artifacts"
	"lerna/authorization"
	"lerna/execution"
...
func open(ctx context.Context, root, token string) (h *harness, err error) {
	...
	h.target, err = simjob.Open(filepath.Join(root, "target.db"), true, true, h.clock)
	if err != nil {
		return nil, err
	}
